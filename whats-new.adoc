...
* Cloud Manager now monitors Kubernetes clusters managed in Cloud Manager. If new persistent volumes are added, Cloud Manager automatically adds them to the list of available volumes. Additionally, backup is automatically enabled if automatic backup was configured on the cluster. 

* When defining storage classes, you can now: 
